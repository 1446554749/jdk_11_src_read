 ...
 *
 * The generated code will provide getters for all elements in the J9JSRIBranchPointer
 * structure.  Where possible, meaningful return types are inferred.
 *
 * The user may add methods to provide meaningful return types where only pointers
 * could be automatically inferred.
 */
@com.ibm.j9ddr.GeneratedPointerClass(structureClass=J9JSRIBranch.class)
...
/*[END USER CODE]*/
	...
	protected J9JSRIBranchPointer(long address) {
		super(address);
	}

	public static J9JSRIBranchPointer cast(AbstractPointer structure) {
	...
	public static J9JSRIBranchPointer cast(long address) {
		if (address == 0) {
			return NULL;
		}
		return new J9JSRIBranchPointer(address);
	}

	public J9JSRIBranchPointer add(long count) {
		return J9JSRIBranchPointer.cast(address + (J9JSRIBranch.SIZEOF * count));
	}

	public J9JSRIBranchPointer add(Scalar count) {
		return add(count.longValue());
	}

	public J9JSRIBranchPointer addOffset(long offset) {
		return J9JSRIBranchPointer.cast(address + offset);
	}

	public J9JSRIBranchPointer addOffset(Scalar offset) {
		return addOffset(offset.longValue());
	}

	public J9JSRIBranchPointer sub(long count) {
		return J9JSRIBranchPointer.cast(address - (J9JSRIBranch.SIZEOF * count));
	}

	public J9JSRIBranchPointer sub(Scalar count) {
		return sub(count.longValue());
	}

	public J9JSRIBranchPointer subOffset(long offset) {
		return J9JSRIBranchPointer.cast(address - offset);
	}

	public J9JSRIBranchPointer subOffset(Scalar offset) {
		return subOffset(offset.longValue());
	}

	public J9JSRIBranchPointer untag(long mask) {
		return J9JSRIBranchPointer.cast(address & ~mask);
	}
	...
	// void** blockParentPointer
	@com.ibm.j9ddr.GeneratedFieldAccessor(offsetFieldName="_blockParentPointerOffset_", declaredType="void**")
