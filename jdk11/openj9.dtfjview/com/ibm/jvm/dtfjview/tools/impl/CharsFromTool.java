...
public class CharsFromTool extends Tool implements IPipe {

	private static final String COMMAND = "charsfrom";
	private static final String ARGUMENT_DESCRIPTION = "[options] <pattern>";
	private static final String HELP_DESCRIPTION = "To be used after a pipeline to remove the characters before a pattern in a line.";
	private static final String USAGE = COMMAND + "\t" + ARGUMENT_DESCRIPTION + "\t" + HELP_DESCRIPTION + "\n" +
			"     Options :\n" +
			"          -e, -exclude : to exclude the matched pattern from the resulting line. If this option is not used, the matched pattern will be included.\n" +
	...
	public void process(String command, String[] args, PrintStream out) throws CommandException {
		Arguments arguments = CharsFromTool.processArguments(args);
		if(arguments == null) {
			out.println(USAGE);
			return;
		}
	...
	private static Arguments processArguments(String[] args) {
		...
		for (;x < args.length; ++x) {
			if(args[x].equalsIgnoreCase("-e") || args[x].equalsIgnoreCase("-exclude")) {
				includeToken = false;
			} else if (args[x].equalsIgnoreCase("-i") || args[x].equalsIgnoreCase("-ignoreCase")) {
				ignoreCase = true;
			} else if (args[x].equalsIgnoreCase("-keep")) {
	...
	private static final class StringModifier implements IStringModifier {
		...
		public String modify(String s) {
			...
			if(index < 0) {
				...
				return s.substring(index + attr.searchToken.length());
			}
		}
